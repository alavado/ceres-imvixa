...
export const calcularNumeroDeBaños = (
  estrategia,
  medicamentos,
  tratamientos,
  curvaCrecimiento
  ...
  return Object.keys(tratamientos[estrategia]).filter((key) => {
    ...
    return (
      medicamento.formaFarmaceutica === FARMACO_APLICACION_BAÑO &&
      key < 1 + Math.floor(curvaCrecimiento.length / 7)
    );
  }).length;
};
...
export const calcularCostoBaños = (
  medicamentos,
  tratamientos,
  numeroDeJaulas,
  volumenJaula
  ...
  return Object.keys(tratamientos).reduce((suma, key) => {
    ...
    if (medicamento.formaFarmaceutica === FARMACO_APLICACION_BAÑO) {
      ...
      const factorKg =
        medicamento.unidadDosis.slice(0, 2) === "ml" ? 1000 : 1000000;
      ...
      const costoBaño =
        (costoProducto + Number(medicamento.costoOperacional)) *
        Number(numeroDeJaulas);
...
export const calcularCostoTratamientoOral = (
  principioActivo,
  medicamentos,
  tratamientos,
  numeroDeSmoltsInicial,
  curvaMortalidadAcumulada,
  curvaCrecimiento,
  bFCR
) => {
  return Object.keys(tratamientos).reduce((suma, semana) => {
    ...
    if (m.principioActivo === principioActivo) {
      ...
      if (semana === "0") {
        const bFCRInicial =
          tratamientos[semana].pesoDeAplicacion < 100 ? 1.001 : (bFCR + 1) / 2;
        pesoPromedio = tratamientos[semana].pesoDeAplicacion / 1000;
        diferenciaPeso = pesoPromedio * 0.2;
        cantidadComida =
          (diferenciaPeso * numeroDeSmoltsInicial) / (1000 * bFCRInicial);
        return (
          suma +
          costoMiligramos * m.dosis * pesoPromedio * numeroDeSmoltsInicial +
          cantidadComida * m.costoOperacional
        );
      }
      const numeroDeSmoltsActual =
        numeroDeSmoltsInicial * (1 - curvaMortalidadAcumulada[numSemana * 4]);
      const diasSemana = [0, 1, 2, 4, 5, 6].map((i) => numSemana * 7 + i);
      const pesosSemana = curvaCrecimiento.filter((v, i) =>
        diasSemana.includes(i)
      );
      pesoPromedio =
        pesosSemana.reduce((prev, current) => (current += prev), 0) /
        (pesosSemana.length * 1000);
      const diaMaximoPeso =
        numSemana * 7 + 6 < curvaCrecimiento.length
          ? numSemana * 7 + 6
          : curvaCrecimiento.length - 1;
      diferenciaPeso =
        (curvaCrecimiento[diaMaximoPeso] -
          curvaCrecimiento[numSemana * 7 - 1]) /
        1000;
      cantidadComida = (diferenciaPeso * numeroDeSmoltsActual) / (1000 * bFCR);
      return (
        suma +
        costoMiligramos * m.dosis * pesoPromedio * numeroDeSmoltsActual +
        cantidadComida * m.costoOperacional
      );
    }
    return suma;
...
export const calcularCostoTratamientosOrales = (
  medicamentos,
  tratamientos,
  numeroDeSmoltsInicial,
  curvaMortalidadAcumulada,
...
export const calcularCostoImvixa = (
  medicamentos,
  tratamientos,
  numeroDeSmoltsInicial,
  curvaMortalidadAcumulada,
    ...
    if (m.esImvixa) {
      ...
      const diaMaximoPeso =
        numSemana * 7 + 6 < curvaCrecimiento.length
          ...
          : curvaCrecimiento.length - 1;
      diferenciaPeso =
        curvaCrecimiento[diaMaximoPeso] - curvaCrecimiento[numSemana * 7 - 1];
      cantidadComida =
        (diferenciaPeso * numeroDeSmoltsActual) / (1000000 * bFCR);
      return (
        suma +
        costoMiligramos * m.dosis * pesoPromedio * numeroDeSmoltsActual +
        cantidadComida * m.costoOperacional
...
export const checkTratamientoEnSemanasAnteriores = (
  tratamientos,
  medicamentos,
  semana,
  principioActivo
  ...
  return semanasMenores.reduce(
    (obj, v) =>
      medicamentos.find((m) => m.id === tratamientos[v].idMedicamento)
        .principioActivo === principioActivo || obj,
    false
  );
};

export const calcularPTI = (medicamentos, tratamientos) => {
  const semanas = Object.keys(tratamientos).sort((a, b) =>
    Number(a) > Number(b) ? 1 : -1
  );
  const pti = semanas.reduce(
    (obj, semana, i) => {
      const { factorFarmaco, factorMetodo, principioActivo } =
        medicamentos.find((m) => m.id === tratamientos[semana].idMedicamento);
      const factorResistencia = checkTratamientoEnSemanasAnteriores(
        tratamientos,
        medicamentos,
        semana,
        principioActivo
...
export const calcularProductosVertidosOrales = (
  medicamentos,
  tratamientos,
  numeroDeSmoltsInicial,
  curvas
  ...
  const semanas = Object.keys(tratamientos).filter((semana) => semana !== "0");
...
export const calcularProductosVertidosBaños = (
  aplicaciones,
  medicamentos,
  volumenJaula,
  numeroDeJaulas
  ...
  return aplicaciones.reduce((productosVertidos, idMedicamento) => {
    ...
    const [numeradorDosis, denominadorDosis] =
      medicamento.unidadDosis.split("/");
    ...
    const cantidadPrincipioActivo =
      gPorCentro * (medicamento.presentacion / 100); // presentacion en porcentaje
    return productosVertidos.concat([
      {
...
export const calcularProductosVertidos = (
  ...
  return calcularProductosVertidosOrales(
    ...
    calcularProductosVertidosBaños(
      aplicaciones,
      medicamentos,
      volumenJaula,
      numeroDeJaulas
...
export const agruparProductosVertidos = (
  medicamentos,
  tratamientos,
  volumenJaula,
  numeroDeJaulas,
  ...
  const principiosActivos = [
    ...new Set(
      medicamentos
        .filter((m) => m.formaFarmaceutica === FARMACO_APLICACION_BAÑO)
        .map((m) => m.principioActivo)
    ),
  ];
  const estrategias = Object.keys(tratamientos);
  const productosVertidos = {
    tradicional: calcularProductosVertidos(
      medicamentos,
    ...
    imvixa: calcularProductosVertidos(
      medicamentos,
      tratamientos.imvixa,
      volumenJaula,
      numeroDeJaulas,
...
export const redondear = (numero, cifrasSignificativas = 1) => {
  return (
    Math.round(numero * 10 ** cifrasSignificativas) / 10 ** cifrasSignificativas
  );
};

export const redondearYAString = (numero, cifrasSignificativas = 1) => {
  return redondear(numero, cifrasSignificativas).toLocaleString("de-DE", {
    minimumFractionDigits: cifrasSignificativas,
    maximumFractionDigits: cifrasSignificativas,
  });
};
